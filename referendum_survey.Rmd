...
mice_model = with(referendum_imputed, glm(referendum ~ age + gender, family = binomial))

pooled_res = pool(mice_model)

pooled_sum <- tidy(pooled_res) %>%
  mutate(term = gsub("gender", "",term))

pooled_sum %>%
  select(term, estimate, std.error, statistic, p.value) %>%
  kable(caption = "Referendum Multiple imputaion (Pooled logistic regression)",
...
Conclusion:
Across all analyses, age was a significant factor, with older people being less likely to vote in favour of the referendum. Being male also ment less chance of voting “yes” compared to the reference group(female). Other gender identities did not show statistically significant differences from the reference group. These findings were the same across both the original and multiple-imputation analyses, suggesting that missing data had little impact on the main results.
